CREATE OR REPLACE PACKAGE BODY "CNL_SYS"."CNL_CTO_UPDATE_SHIPMENT_PCK" 
is
/**********************************************************************************
* $Archive: $
* $Revision: $ 1  
...
--
	...
	g_pck		varchar2(30) 			:= 'cnl_cto_update_shipment_pck';
	...
	type	g_parcel_rec	is record( parcel_id		dcsdba.order_container.pallet_id%type
					 ...
					 , typeofpackage	dcsdba.pallet_config.pallet_type_group%type
...
------------------------------------------------------------------------------------------------
-- Author  : M. Swinkels 24/05/2022
-- Purpose : Build route number
------------------------------------------------------------------------------------------------
function build_route_number( p_carrier_id_i 	in varchar2
			   , p_service_level_i	in varchar2
			   , p_drop_point_i	in varchar2
			   , p_country_i	in varchar2
			   )
	return varchar2
is
	l_rtn		varchar2(30) := 'build_route_number';
	l_retval	varchar2(35);
	l_fixed_val	varchar2(5) := 'BBX'||p_country_i;
	l_drop_point	dcsdba.order_header.delivery_point%type;
	l_date		varchar2(4);
	l_time		varchar2(6) := to_char(sysdate,'hh24miss');
begin
	-- Add logging
	cnl_sys.cnl_cto_pck.create_cto_log_record( g_pck||'.'||l_rtn
						 , 'Start building route number'
						 );
	if	p_service_level_i != 'BBX'
	then
		-- Add logging
		cnl_sys.cnl_cto_pck.create_cto_log_record( g_pck||'.'||l_rtn
							 , 'Not a BBX shipment so no routenumber created'
							 );

		return null;
	else
		if 	p_carrier_id_i = 'STD.DHL.COM'
		then
			l_retval := 'DHL-BBX-'||p_country_i;
		else
			l_retval := substr(p_carrier_id_i,1,3)||'-'||substr(p_service_level_i,1,3)||'-'||p_country_i;
		end if;

		if	p_country_i = 'NO'
		then
			if	p_drop_point_i = 'SVG'
			then
				l_drop_point	:= 'SVG';
			else
				l_drop_point	:= 'OSL';
		...
		-- Cut off time is 16:00 so after 16 ship by date will be next day.
		if	l_time >= 160000
		then
			l_retval	:= substr(l_retval||'-'||l_drop_point||'-'||to_char(sysdate+1,'YYYYMMDD'),1,25);
		else
			l_retval	:= substr(l_retval||'-'||l_drop_point||'-'||to_char(sysdate,'YYYYMMDD'),1,25);
		end if;

		-- Add logging
		cnl_sys.cnl_cto_pck.create_cto_log_record( g_pck||'.'||l_rtn
							 , 'Routenumber '||l_retval||' created'
							 );
		return l_retval;
	end if;
exception 
...
begin
	...
	if	g_order_header.ship_by_date is not null
	and	g_order_header.ship_by_date > sysdate
	then
		l_retval	:= true;
...
begin
	...
	l_att(l_att.count).value	:= g_database;
...
begin
	...
	l_address(l_address.count).carr_account_nbr		:= g_order_header.hub_vat_number; -- Misused field in WMS to set clients account number
...
begin
	...
	if	g_order_header.delivery_point is not null
	then
		l_delivery_point	:= build_route_number( p_carrier_id_i 		=> g_order_header.carrier_id
							     , p_service_level_i	=> g_order_header.service_level
							     , p_drop_point_i		=> g_order_header.delivery_point
							     , p_country_i		=> iso2_country(g_order_header.del_country)
							     );
		if	l_delivery_point is not null
		then
			g_shipment.put('routeNumber',			l_delivery_point);
		end if;
	end if;
	...
	-- transport payer customer number
...
------------------------------------------------------------------------------------------------
procedure process_updshipment_response_p( p_json_string_i	in pljson)
is
	l_service_level		dcsdba.order_header.service_level%type;
	l_rtn			varchar2(30) 	:= 'process_updshipment_response_p';
...
begin 
	...
	then
		delete	cnl_cto_shipments_to_update u
		where	(	u.run_id 	= g_run_id
			and	u.site_id 	= g_order_header.from_site_id
			and	u.client_id	= g_order_header.client_id
			and	u.order_id 	= g_order_header.order_id
			and	(u.shipment_id 	= g_order_header.shipment_id or u.shipment_id is null)
			)
		or	(	u.has_error 	= 'Y' 
			and	(
				select	count(*)
				...
				and	o.from_site_id 		= u.site_id
				and	o.client_id		= u.client_id
				and	(o.uploaded_ws2pc_id 	= u.shipment_id or u.shipment_id is null)
				) = 0 -- order no longer exists so clean up error records
			)
		or	(	nvl(u.has_error,'X')	= 'X'
			and	u.run_id 	is not null
			and	u.run_id 	!= g_run_id
			and	(
				select	count(*)
				from	dcsdba.order_header o
				where	o.order_id 		= u.order_id
		...
		update	cnl_cto_shipments_to_update u
		set 	has_error = 'Y'
		where	u.run_id 	= g_run_id
		and	u.site_id 	= g_order_header.from_site_id
		and	u.client_id	= g_order_header.client_id
...
end process_updshipment_response_p;
...
is
	l_url			varchar2(1000)	:= cnl_util_pck.get_constant('CTO_UPDATE_SHIPMENT_WEBSERVICE_URL');
...
begin
	...
	loop
		...
		-- Add logging
		cnl_sys.cnl_cto_pck.create_cto_log_record( g_pck||'.'||l_rtn
							 , 'Start update shipment procedure for order_id '
							 || g_order_header.order_id
							 || ' from client '
							 || g_order_header.client_id
							 || ' with shipment id '
		...
		process_updshipment_response_p( l_body_response);
...
------------------------------------------------------------------------------------------------
procedure fetch_orders_updship_p
is
	cursor c_orders (b_run_id integer)
	is
		...
		inner
		join	cnl_cto_shipments_to_update u
		on	u.site_id		= o.from_site_id
		and	u.client_id		= o.client_id
		and	u.order_id		= o.order_id
		and	(u.shipment_id		= o.uploaded_ws2pc_id or u.shipment_id is null)
		and	u.run_id		= b_run_id
		order
		by	o.creation_date 	asc
	;

	l_cnt				integer 	:= 0;
	l_rtn				varchar2(30) 	:= 'fetch_orders_updship_p';
	l_ins				integer		:= 0;
	l_run_id			integer		:= shipment_update_runid_seq1.nextval;
	l_limit				integer		:= 5;
...
begin
	--Set run id for all orders that require an update
	update	cnl_cto_shipments_to_update
	set	run_id = l_run_id
	...
	loop
		...
		then
			-- Add logging
			cnl_sys.cnl_cto_pck.create_cto_log_record( g_pck||'.'||l_rtn
								 , 'Start updating Centiro shipments with Run id = '
								 || to_char(l_run_id)
								 ||'.'
								 );
			...
			-- Add logging
			cnl_sys.cnl_cto_pck.create_cto_log_record( g_pck||'.'||l_rtn
								 , 'Found '||to_char(g_ord.count)||' shipments to update with run id = '
								 || to_char(l_run_id)
								 ||'.'
								 );
		end if;
...
begin
	null;
end cnl_cto_update_shipment_pck;